...
def read_mast_data(fpath):
    ...
    with open(fpath) as csvfile:
        ...
        for row in rows:
            masts.append(
                PhoneMast(
                   prop_name=row[0], prop_addr1=row[1],
                   prop_addr2=row[2], prop_addr3=row[3], prop_addr4=row[4],
                   unit_name=row[5], tenant=row[6],
                   lease_start_date=datetime.strptime(row[7], date_fmt).date(),
                   lease_end_date=datetime.strptime(row[8], date_fmt).date(),
...
if __name__ == '__main__':
    ...
    parser.add_argument(
            '--five-by-rent-asc', dest='show_by_rent',
            default=False, action='store_true',
            help='Show first 5 masts sorted by "Current Rent"')
    parser.add_argument(
            '--lease-years', dest='lease_years_25',
            default=False, action='store_true',
            help='Show masts with 25 lease years')
    parser.add_argument(
            '--mast-count-by-tenant', dest='count_by_tenant',
            default=False, action='store_true',
            help='Show mast count by tenant')
    parser.add_argument(
            '--lease-date-between', dest='lease_date_between',
            default=False, action='store_true',
    ...
    if args.lease_years_25:
        ...
        print('\n2a) Output the list to the console, ''including all data '
              'fields\n')
        print(masts_with_25_lease_years)
        print('\n2b) Output the total rent for all items in this list to the '
              'console\n')
    ...
    if args.count_by_tenant:
        print('\n3a) Output the dictionary to the console in a readable '
              'form\n')
        pprint(dict(phone_masts.count_by_tenant()))

    if args.lease_date_between:
        print('\n4) List the data for rentals with “Lease Start Date” between '
              '1st June 1999 and 31st August 2007\n')
        print('\n4a) Output the data to the console with dates formatted as '
              'DD/MM/YYYY\n')
        print(phone_masts.lease_start_between(
            date(1999, 6, 1), date(2007, 8, 31)))
